...
class WeverseClient {
  ...
  async getPosts(communityId, page = 1, size = 20) {
    try {
      logger.info(`Getting posts for community: ${communityId}, page: ${page}, size: ${size}`);
      // Use the correct endpoint for posts
      const response = await this.axiosInstance.get(`/v1/community/${communityId}/posts`, {
        params: { 
          page, 
          size, 
          sort: 'RECENT'
      ...
      if (error.response && error.response.status === 401) {
        ...
        if (refreshSuccessful) {
          // Try the request again
          return this.getPosts(communityId, page, size);
        }
      }
...
(async () => {
  ...
  if (!process.env.WEVERSE_EMAIL || !process.env.WEVERSE_PASSWORD) {
    ...
    if (!process.env.WEVERSE_EMAIL) {
      logger.debug('Setting fallback WEVERSE_EMAIL for testing');
      process.env.WEVERSE_EMAIL = 'stokmlpipzstore01@gmail.com';
    }
    
    if (!process.env.WEVERSE_PASSWORD) {
      logger.debug('Setting fallback WEVERSE_PASSWORD for testing');
